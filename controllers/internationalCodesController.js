...
const getInternationalCode = async (req, res) => {
  try {
    const Illness = await Models.InternationalCodes.findOne({
...
const getInternationalCodeByCode = async (req, res) => {
  try {
    let Illness = await Models.InternationalCodes.findOne({
      where: { code: req.body.code },
    });
    Illness ??= {
      message: "Illness with such international code was not found",
    };
    res.status(200).json(Illness);
  } catch (err) {
    res.status(500).json({
...
const updateInternationalCode = async (req, res) => {
  try {
    const Illness = await Models.InternationalCodes.update(req.body, {
      where: { id: req.params.id },
    });
    res.status(200).json(Illness);
  } catch (err) {
